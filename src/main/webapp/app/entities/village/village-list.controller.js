...
angular.module('talarionApp')
    .controller('VillageController', function($scope, $state, Village, ParseLinks, growl) {
        $scope.villageList = [];
        $scope.page = 1;
        ...
        $scope.showUpdate = function(id) {
            Village.get({
                ...
                $('#saveVillageModal').modal('show');
            });
        };

        $scope.save = function() {
            if ($scope.author.id != null) {
                Village.update($scope.village,
                    function() {
                        $scope.refresh();
                    });
            } else {
                Village.save($scope.village,
                    function() {
                        $scope.refresh();
                    });
            }
        ...
        $scope.delete = function(id) {
            Village.get({
                ...
                $scope.village = result;
                $('#deleteVillageConfirmation').modal('show');
            });
        };
        ...
        $scope.confirmDelete = function(id) {
            Village.delete({
                ...
                function() {
                    $scope.loadAll();
                    $('#deleteVillageConfirmation').modal('hide');
                    $scope.clear();
                    growl.info("Village successfully deleted ", {});
                });
        };

        $scope.refresh = function() {
            $scope.loadAll();
            $('#saveVillageModal').modal('hide');
            $scope.clear();
        };
